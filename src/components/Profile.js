    ...
    if (user.name) {
      return (
        <div id="profile">
          ...
          {user.reviews && user.reviews.length > 0 ? <ReviewsContainer reviews={user.reviews}/> : <h2>You don't have any reviews yet! <Link to="/hospitals">Click here</Link> to see hospitals.</h2> }
        </div>
      )
    } else {
      return null
